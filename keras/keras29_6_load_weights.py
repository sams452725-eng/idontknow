...
# model.save_weights(path + 'keras29_5_save_1.weights.h5')      # 초기 가중치. 실질적으로 무의미!

# model = load_model(path + 'keras29_1_save_model.keras')
# model.load_weights(path + 'keras29_5_save_1.weights.h5')      # save_weights는 w값만 저장이 되고 모델이 저장되는건 아니기 때문에 모델이 살아있어야 한다.

model.load_weights(path + 'keras29_5_save_2.weights.h5')        # 최적의 가중치

# 가중치를 저장해서 사용할때는 모델과 컴파일이 무조건 있어야한다.
